...
// ✅ Global canvas manager - hanya 1 canvas yang bisa aktif
...
export const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  // Listen to global canvas changes
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      // Close this canvas if another one is opened
  ...
  // ✅ Listen for settings opened event
  useEffect(() => {
    const handler = () => {
      // Close canvas when settings is opened
      if (showCanvas) {
        setShowCanvas(false);
  ...
  // ✅ NEW: Listen for conversation changed event
  useEffect(() => {
    const handler = () => {
      // Close canvas when conversation is changed
      if (showCanvas) {
        setShowCanvas(false);
        EMIT_CANVAS_CHANGE(null, 0);
      }
  ...
  // Auto-open canvas when NEW code arrives (only if no other canvas is open)
  useEffect(() => {
  ...
  const handleOpenCanvas = () => {
    // Close any other open canvas first
  ...
  // Check if this canvas is the active one
  ...
  return (
    <>
      <div className="flex flex-col gap-2">
        ...
        {/* Canvas Button - Only show if this canvas is not active */}
      ...
      {/* Canvas - Only render if this is the active canvas */}
